...
const repeatMyParam = (str) => {
    ...
    axios.get('http://localhost:3000/repeat/NewString')
    .then((response) => {
        console.log(response.data)
        let repeatTextContent = document.querySelector('#repeat-text')
        repeatTextContent.textContent = response.data;
        repeatTextContent.style.display = 'block';
    })
